<?php
    ...
    if (!isset($_POST['city']) || empty($_POST['city']) || !isset($_POST['newname']) || empty($_POST['newname'])) {
        header("Location: /administration.php");
        exit;
    } else {
        $error = 0;
        $city = mysql_escape_string(htmlspecialchars(trim($_POST['city'])));
        $newname = mysql_escape_string(htmlspecialchars(trim($_POST['newname'])));
        $query = "UPDATE cities SET name = '$newname' WHERE cities.id = $city";
